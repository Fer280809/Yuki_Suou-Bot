const handler = async (m, {conn}) => {
  ...
  const hsl = `
*✧ Resultados de la Pezaca ${conn.getName(m.sender)} ✧*
 ...
 *🐡 ${ar6} ${anti6}*		    *🐚 ${ar12} ${anti12}*`.trim();
  ...
  if (new Date - global.db.data.users[m.sender].lastberburu < 2700000) return conn.reply(m.chat, `Por favor descansá un momento para seguir Pezcando\n\n⫹⫺ Tiempo ${clockString(time - new Date())}\n${wm}`, m);
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['Armas lista para la Pezca!!', 'Probando Armas 🪡 💣 🪓 🏹 🧹', 'VARCOS PARA LA PEZCA!!  🚤 🛥️ 🛶 ⛴️', 'TIEMPO BUENO PARA LA PEZCA 🧤'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 15000);

  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['Buscando Lugares de Pezca...', 'Alistando todo para la Pezca!!', 'Estableciendo Lugar de la Pezca...', 'PREPARANDO LUGAR DE Pezca!!'].getRandom()}*`, m, m.mentionedJid ? {mentions: [m.sender]} : {});
  }, 0);
  user.lastberburu = new Date * 1;
};
handler.help = ['berburu'];
handler.tags = ['rpg'];
handler.command = ['pezca', 'pez', 'pezcar']
handler.group = true;
